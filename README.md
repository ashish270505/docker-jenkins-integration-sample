...
- Creates a docker image that runs a Java [main method](src/main/java/com/mycompany/myapp/HelloWorld.java) on start of the container and terminates after the program is finished
...
- Uses the fabric8 [maven-docker-plugin](https://github.com/fabric8io/docker-maven-plugin)
  - Build the image
  - Start a container for an [integration test](src/test/java/com/mycompany/myapp/HelloWorldIT.java)
...
```
REPOSITORY                        TAG
mycomp/docker-jenkins-integration-sample     0.1.0-SNAPSHOT
mycomp/docker-jenkins-integration-sample     latest
```
...
```
docker run \
--volume "$(pwd)"/data:/usr/src/docker-jenkins-integration-sample/data \
--user $UID:$UID \
--env my_var=abc123 \
mycomp/docker-jenkins-integration-sample
```
